...
// Build id (debug)
try { console.info('[TRON] tron-ares.ios-inline.js build: link-status-tagsright-titlemore@2026-01-17'); } catch {}

// Force l'alignement du badge link-status (OK/KO) a droite et maximise la place du titre
// IMPORTANT: applique uniquement a la liste Films (channelList) via data-type="channels"
(function __injectChannelListLinkStatusRightCSS(){
  try {
    const STYLE_ID = 'tron-ares-linkstatus-right-css';
    if (document.getElementById(STYLE_ID)) return;
    const st = document.createElement('style');
    st.id = STYLE_ID;
    st.textContent = `
      /* Dans channelList (Films): place le badge OK/KO a droite de la ligne des tags */
      .channel-item[data-type="channels"] .channel-tags{ width:100% !important; align-items:center !important; }
      .channel-item[data-type="channels"] .channel-tags .link-status{ margin-left:auto !important; }

      /* Dans channelList (Films): donne plus de place au titre (1 ligne) */
      .channel-item[data-type="channels"]{ gap:7px !important; padding:6px 8px !important; }
      .channel-item[data-type="channels"] .channel-title-row{ gap:6px !important; }
      .channel-item[data-type="channels"] .channel-title{ font-size:11px !important; }
      .channel-item[data-type="channels"] .channel-actions{ gap:3px !important; }
      .channel-item[data-type="channels"] .icon-btn{ width:20px !important; height:20px !important; font-size:11px !important; }
    `;
...
function renderChannelList() {
  ...
  if (newAdditionsMode) {
    ...
    idxs.forEach((idx) => {
      ...
      __attachSynopsisClickHandlersToChannelList();
    return;
  }
...
function createChannelElement(entry, index, sourceType) {
  ...
  // Rend le titre prioritaire en largeur et laisse la place au badge a droite
  try { titleDiv.style.flex = '1 1 auto'; } catch {}
  try { titleDiv.style.minWidth = '0'; } catch {}
  ...
  // Badge link-status: par defaut a droite dans son conteneur
  try { statusBadge.style.marginLeft = 'auto'; } catch {}
  ...
  if (sourceType !== 'channels') titleRow.appendChild(statusBadge);
  ...
  if (sourceType === 'channels') {
    // Place le badge OK/KO a droite de la ligne des tags (libere le titre)
    try { tagsDiv.style.width = '100%'; } catch {}
    try { tagsDiv.style.alignItems = 'center'; } catch {}
    try { statusBadge.style.marginLeft = 'auto'; } catch {}
