...
function toggleShowcaseOverlay(){
  if(!showcaseOverlay) return;
  if(showcaseOverlay.classList.contains('hidden')) openShowcaseOverlay();
  else closeShowcaseOverlay();
}
...
function buildShowcase(){
  ...
  if(showcaseSub) showcaseSub.textContent = total ? `${total} films — par catégories` : 'Aucun film chargé';

  if(!total){
    const empty = document.createElement('div');
    empty.className = 'showcase-empty';
    empty.textContent = "Aucun film n'est chargé pour le moment. Charge une playlist Films (M3U) et réessaie.";
    showcaseBody.appendChild(empty);
    return;
  }

  // Group by category (group-title)
  const grouped = new Map(); // groupName -> [{entry, index}]
  for(let i=0;i<channels.length;i++){
    const ch = channels[i];
  ...
  for(const [groupName, items] of grouped.entries()){
    ...
    for(const it of items){
      ...
      if(logo && logo.type === 'image' && typeof logo.value === 'string' && logo.value.trim()){
        ...
        const v = (logo && logo.type === 'letter' && logo.value) ? String(logo.value) : String(entry?.name || '?').trim().slice(0,1).toUpperCase();
...
(() => {
  ...
  if(showcaseBtn){
    showcaseBtn.addEventListener('click', () => {
      updateShowcaseButtonVisibility();
      toggleShowcaseOverlay();
    });
  }

  if(showcaseCloseBtn){
    showcaseCloseBtn.addEventListener('click', () => closeShowcaseOverlay());
  }

  if(showcaseOverlay){
    showcaseOverlay.addEventListener('click', (e) => {
      // clic sur le fond -> close
      if(e && e.target === showcaseOverlay) closeShowcaseOverlay();
    });
  }

  document.addEventListener('keydown', (e) => {
    if(!showcaseOpen) return;
    if(e.key === 'Escape') closeShowcaseOverlay();
  });

  // quand on change d'onglet: cache/affiche
  document.addEventListener('click', (e) => {
    if (e.target && e.target.closest && e.target.closest('.tab-btn')) {
