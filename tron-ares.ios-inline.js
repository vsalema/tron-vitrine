...
// =====================================================
// 🎞️ VITRINE (Films par catégories) - overlay dans le player
// =====================================================
let showcaseOpen = false;

function _groupChannelsForShowcase(){
  // Construit des sections par catégorie (entry.group) en conservant l'ordre d'apparition
  const sections = [];
  const map = new Map();
  ...
  // Optionnel: une section "Derniers ajouts" en premier
  try {
    const idxs = computeNewAdditionsIndexes();
    if (Array.isArray(idxs) && idxs.length) {
      const sec = ensure('Derniers ajouts');
      for (const idx of idxs) {
        if (typeof idx === 'number' && idx >= 0 && idx < channels.length) {
          sec.items.push({ index: idx, entry: channels[idx] });
        }
      }
    }
  } catch {}
  ...
  for (let i = 0; i < channels.length; i++) {
    ...
    if (!entry) continue;
    const title = (entry.group || 'Autres').trim() || 'Autres';
    ensure(title).items.push({ index: i, entry });
  }

  return sections.filter(s => s && Array.isArray(s.items) && s.items.length);
}
...
function openShowcase(){
  ...
  // Render
  const sections = _groupChannelsForShowcase();
  showcaseBody.innerHTML = '';

  if (!sections.length) {
    const empty = document.createElement('div');
    empty.style.cssText = 'padding:10px; font-size:12px; color:rgba(255,255,255,.85);';
    empty.textContent = 'Aucun film à afficher (charge ta playlist Film M3U).';
    ...
    for (const sec of sections) {
      ...
      count.textContent = String(sec.items.length).padStart(2, '0');
      ...
      for (const it of sec.items) {
        ...
        const idx = it.index;
        if (!entry || typeof idx !== 'number') continue;
        ...
        if (logo && logo.type === 'image' && typeof logo.value === 'string' && logo.value.trim()) {
          ...
          img.loading = 'lazy';
          img.decoding = 'async';
          img.alt = entry.name || 'Poster';
          img.src = logo.value;
          poster.appendChild(img);
        } else {
          const letter = document.createElement('div');
          letter.className = 'showcase-letter';
          const val = (logo && logo.type === 'letter' && logo.value) ? logo.value : (entry.name || '•').slice(0,1);
          letter.textContent = String(val).slice(0,1).toUpperCase();
        ...
        sub.textContent = (entry.group || 'Autres');
...
// =====================================================
(() => {
  // Visible uniquement en onglet Films
  ...
  // Clic en dehors du contenu → fermer (overlay plein cadre)
  if (showcaseOverlay) {
  ...
  // Changement d'onglet → on masque/affiche le bouton + on ferme si besoin
  document.addEventListener('click', (e) => {
    if (e.target && e.target.closest && e.target.closest('.tab-btn')) {
